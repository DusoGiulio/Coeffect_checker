...
public class Fill_STC_STM  {
		...
		for(NodeAST klass :  listClass) 
		{
			if(klass instanceof ClassDecl) 
			{
				...
				if(this.getClassST().lookup(id.getName())==null) 
				{
					...
					{
						ClassAttribute attr= new ClassAttribute(new SymbolTable(), c.getIdextends().getName(),c.getType());
					...
					{
						ClassAttribute attr= new ClassAttribute(new SymbolTable(), null, c.getType());
						if(((ClassTypeDescriptor)c.getType()).isCoeff()!=Coef.NOTCOEF) 
						{
							attr.setIsCoeff(((ClassTypeDescriptor)c.getType()).isCoeff());
						}
			...
			else if(klass instanceof MainClass)
			{
				...
				ClassAttribute attr= new ClassAttribute(st, id.getName(), c.getType());
				this.getClassST().enter(id.getName(), attr);
			}
		}
				...
				{//se il nodo ast contiene una classe in extend
					...
					{//se la classe esiste nella symboltable delle classi
						...
						{//invoco la funzione che sarà ricorsiva passandogli il nome della classe e riempio this.extClass con le classi che estende la classe attuale
							...
							boolean flag = false;
							SymbolTable abstractMethod = new SymbolTable();
							
							while(i>=0) 
							{//prendo la symboltable della stringa iesima
								ClassAttribute cAtt=this.getClassST().lookup(this.extClass.get(i));
								SymbolTable extSymTab=cAtt.getST();
								//se la classe descritta dal nome this.extClass.get(i) è astratta  allora inserisco in una symboltable temporanea tutti i metodi astratti
								if(((ClassTypeDescriptor)cAtt.getType()).getIsAbstract())
								{
									abstractMethod =this.getAbstractMethod(extSymTab,abstractMethod);
								}
								
								if(i>0) 
								{
									//verifico che le due symbol table siano diverse 
									ClassAttribute targetSymbTab= this.getClassST().lookup(this.extClass.get(i-1));
										for(NodeId Id: targetSymbTab.getST().getSymbolTable().keySet()) 
										{
											...
											{
												if(this.SSCheckExist(new FieldDecl(Id.getType(),Id,0), extSymTab,targetSymbTab.getST())) 
												{
										...
										if(!flag) 
										{
											...
											{
												if(nid.getType() instanceof MethTypeDescriptor){
													if(!((MethTypeDescriptor)nid.getType()).isAbstract()){
								...
								//controllo che la classe attuale contenga tutti i metodi astratti delle classi gerarchicamente superiori a lei
								if(!this.checkAbstarctMethodExist(abstractMethod,this.getClassST().lookup(c.getIdClass().getName()).getST())) 
								{
									System.err.println("La classe "+ c.getIdClass().getName() + " non implementa tutti i metodi astratti" );
									throw new SemanticException(c.getLine());
								}
								i--;
							}
	...
	private boolean checkAbstarctMethodExist(SymbolTable abstractMethod, SymbolTable st) {
		for(NodeId met: abstractMethod.getSymbolTable().keySet()) {
			boolean flag= false;

			for(NodeId node: st.getSymbolTable().keySet()) {
				if(node.getType() instanceof MethTypeDescriptor) {
					if(met.getName().equals(node.getName())) 
					{
						flag=true;
					}
				}
	...
	private SymbolTable getAbstractMethod(SymbolTable extSymTab, SymbolTable abstractMethod) {
		for(NodeId node: extSymTab.getSymbolTable().keySet()) 
		{
			if(node.getType() instanceof MethTypeDescriptor) 
			{
				if(((MethTypeDescriptor)node.getType()).isAbstract()) 
				{
					abstractMethod.enter(node, extSymTab.lookup(node) );
				}
			}
		}
	...
	public void visitClassDecl(ClassDecl ctx) throws SemanticException 
	{
		...
		{//faccio partire una visita del metodo
			if(!this.SSCheckExist(new FieldDecl(met.getType(),met.getId(),0), this.getClassST().lookup(ctx.getIdClass().getName()).getST(), null)) 
			{
				...
				//setto il campo dei formals nel metodo
